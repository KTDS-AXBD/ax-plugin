...
| | `daily-check` | "환경 점검", "daily check" | 환경 점검 + SPEC.md 수치 정합성 자동 보정 |
| | `session-end` | "세션 종료" | 수치 동기화 + 코드 커밋 + 문서 갱신 + git push + CI/CD 배포 |
| **코드** | `code-verify` | "코드 검증", "lint", "test" | lint + typecheck + test 통합 실행 |
| | `code-deploy` | "프리뷰 배포" | 프리뷰 배포 또는 명시적 재배포 |
| | `e2e-audit` | "E2E", "playwright" | E2E 실행 + 감사 + 커버리지 매트릭스 |
...
| | `sprint-watch` | "sprint watch", "gist" | Sprint WT 원격 모니터링 + Gist 주기 갱신 |
...
| **Task** | `task` | "task start", "task list" | F/B/C/X 4트랙 task orchestrator (S-α MVP) — Master pane 기반 WT 생성 + GitHub Issue |
| **하네스** | `init-harness` | "init-harness", "신규 프로젝트 초기화" | Tier 1 설치: 3단계(preflight→install→verify) 4-package monorepo 골격 |
