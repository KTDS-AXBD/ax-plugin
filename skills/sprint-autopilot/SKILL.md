...
4. **Rubric 확인 (opt-in, Sinclair Loop v1)**: `docs/01-plan/features/sprint-{N}.rubric.md` 존재 여부 확인
   - **있으면**: Step 5에서 rubric 항목별 채점이 활성화됨을 인지 (rubric은 Plan 단계에서 작성된 것만 사용 - 구현 후 작성하면 구현에 맞춘 기준이 되어 검증 독립성 훼손)
   - **없으면**: silent skip (파일럿 기간 opt-in - rubric 없는 sprint는 기존 Match Rate 단일 기준)
   - rubric 형식: `| # | 기준 (checkable) | 검증 방법 | 통과 |` 표, 5~9개 항목, **전부 기계 검증 가능해야 함** ("코드가 깔끔하다" 같은 주관 기준 금지)
5. `.sprint-context`에 `CHECKPOINT=plan` 기록
...
2b. **Rubric 채점** (`docs/01-plan/features/sprint-{N}.rubric.md` 존재 시, Sinclair Loop v1):
   - gap-detector(grader)가 Match Rate 산출과 **함께** rubric 항목별 ☑/☐를 채점한다 (검증 방법 칼럼대로 실측: vitest/typecheck/playwright/파일 존재 등)
   - 종료 조건이 `Match Rate >= 90%` → **`Match Rate >= 90% AND rubric 전 항목 ☑`** 로 강화됨
   - 미충족 ☐ 항목이 iterate의 **구체적 수정 목표**가 된다 (점수만 보고 헤매는 것 방지)
3. **Match Rate >= 90%** (+rubric 존재 시 전 항목 ☑): → Step 6 (Report)
4. **Match Rate < 90%** (또는 rubric ☐ 잔존):
   - **Tier 1/2**: `/pdca iterate sprint-{N}` 실행 (최대 3회)
   - **Tier 3**: Gap 목록 기반 직접 수정 후 재분석
   - **LESSONS 🔴 기록**: iterate가 발생하면 그 갭(증상+원인 추정)을 프로젝트 루트 `LESSONS.md`에 🔴 행으로 즉시 추가 (파일 존재 시. 실패는 그 자리에서 잡는 게 가장 싸다 - Fail/Investigate 단계)
