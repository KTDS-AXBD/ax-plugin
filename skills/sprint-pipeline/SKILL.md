...
```python
{
  ...
  "phase6": {"status": "pending", "average_match_rate": null, "prd_match_rate": null, "gap_sprints": [], "results": {}, "e2e_audit": {"route_coverage": null, "feature_coverage": null, "high_gaps": [], "quality_issues": 0}},
  "phase7": {"status": "pending", "gap_fix": {"sprints_processed": 0, "results": {}}, "e2e_fix": {"tests_added": 0, "specs_modified": 0}},
...
### Phase 6: 통합 Gap Analysis + E2E Audit (PDCA Check)

Phase 5 완료 후, Master에서 **Design↔Implementation Gap** + **E2E 커버리지 감사**를 통합 실행한다.
개별 Sprint autopilot의 Gap이 아닌, merge 후 master 기준의 **Phase 전체** 정합성을 검증한다.

> **v3 변경 (S271)**: Phase 6을 6a(Gap) + 6b(E2E Audit)로 분리. 순차 Sprint에서도 `/pdca analyze sprint N~M`과 `/ax:e2e-audit sprint N~M` 패턴을 자동 적용.

**6a. 통합 Gap Analysis:**

`gap-detector` 에이전트를 호출한다. 전체 Sprint + PRD Must Have를 한 번에 분석:

```
Agent(subagent_type="bkit:gap-detector") 호출:
- Design 문서: docs/02-design/features/sprint-{N}.design.md (각 Sprint별)
- PRD: 해당 Phase의 PRD 문서 (docs/specs/*/prd-final.md)
- 구현 코드: merge된 master의 실제 코드
- 분석 기준:
  1. 각 Sprint Design 항목별 구현 여부
  2. PRD Must Have (M1~Mn) 기능별 완료 여부
  3. 누락/부분구현/의도적제외 분류
  4. Added/Changed Features (Design에 없지만 구현된 것)
```

분석 결과에서 Sprint별 + PRD별 Match Rate를 추출:
...
Gap Analysis 직후, E2E 커버리지 감사를 실행한다:

```
E2E Audit 수행 항목:
1. 라우트 커버리지: Phase에서 추가된 라우트 vs E2E spec에서 goto()하는 라우트
2. 기능 커버리지: 각 F-item의 핵심 사용자 경로(submit 폼, trace 검색 등)가 E2E로 검증되는지
3. 품질 분석: waitForTimeout/약한assertion/API-only 등 anti-pattern 감지
```
...
| E2E Gap 심각도 | 동작 |
|:-----------:|------|
| HIGH (핵심 경로 미커버) | Phase 7에서 E2E 자동 추가 대상 |
| MEDIUM (보조 기능 미커버) | 권장사항으로 기록 |
| LOW (공개 API 미커버) | 권장사항으로 기록 |

**6c. 분석 문서 저장:**

`docs/03-analysis/features/phase-{N}-pipeline.analysis.md`에 통합 분석 결과를 저장한다.
Gap Analysis + E2E Audit 결과를 하나의 문서에 병합.
...
```json
{
  "phase6": {
    "status": "done",
    "average_match_rate": 94,
    "gap_sprints": [],
    "results": { "sprint_273": 97, "sprint_274": 95, "sprint_275": 93 },
    "prd_match_rate": 88,
    "e2e_audit": {
      "route_coverage": "3/3 (100%)",
      "feature_coverage": "4/7 (57%)",
      "high_gaps": ["SubmitTab E2E", "TraceTab E2E"],
      "quality_issues": 0
    }
  }
}
...
Gap Sprint가 0건 **AND** E2E HIGH gap이 0건이면 "✅ Check 완료 — Phase 7 건너뜀" 출력 후 Phase 8로 직행.

### Phase 7: Auto Correction (Gap Fix + E2E 보강)

Phase 6에서 식별된 Gap + E2E 미커버를 자동 보정한다.
Gap Sprint도 없고 E2E HIGH gap도 없으면 이 Phase는 건너뛴다.

> **v3 변경 (S271)**: Gap Iterator + E2E 보강을 통합. Design↔코드 gap과 E2E 커버리지 gap을 한 Phase에서 처리.

**7a. Design Gap 보정 (pdca-iterator):**

Phase 6a에서 Match Rate < 90% Sprint가 있으면 `pdca-iterator` 에이전트를 호출:

```
Agent(subagent_type="bkit:pdca-iterator") 호출:
- feature: sprint-{N}
- Design 문서: docs/02-design/features/sprint-{N}.design.md
- Gap 목록: Phase 6a에서 식별된 gap items
- 최대 iteration: 5회
- 목표: Match Rate >= 90%
```

**7b. E2E 보강 (e2e-audit fix 패턴):**

Phase 6b에서 HIGH gap이 있으면 E2E 테스트를 자동 추가:

```
E2E 보강 절차:
1. HIGH gap 목록에서 대상 spec 파일 + 추가할 테스트 식별
2. 기존 spec 파일의 mock 패턴 + fixture를 참조하여 테스트 작성
3. mock 데이터: F-item의 API 스키마(Zod)에서 자동 생성
4. 추가 후 해당 spec만 재실행하여 PASS 확인
   (dev 서버 미실행 시 mock-only 테스트로 작성 — CI에서 검증)
5. 커밋: "test(e2e): F{N} {기능명} E2E 추가 (pipeline Phase 7b)"
```

**7c. 결과 수집 + 커밋:**

Gap fix + E2E 보강 결과를 한 번에 커밋:
- 수정된 구현 파일 (7a 결과)
- 추가된 E2E spec (7b 결과)
- 커밋 메시지에 Phase 7 자동 보정임을 명시

**7d. Pipeline State 갱신:**
...
```json
{
  "phase7": {
    ...
    "gap_fix": {
      "sprints_processed": 1,
      "results": {
        "sprint_N": { "initial": 85, "final": 93, "iterations": 2, "verdict": "improved" }
      }
    },
    "e2e_fix": {
      "tests_added": 5,
      "specs_modified": 2,
      "feature_coverage_before": "4/7 (57%)",
      "feature_coverage_after": "7/7 (100%)"
    }
  }
}
```
...
**7e. 보정 실패 처리:**
- Gap: 5회 iterate 후에도 < 90%: `⚠️ Sprint {N}: {rate}% — 수동 보완 권장`
- E2E: 테스트 추가 후 FAIL: `⚠️ {spec}: 자동 작성 실패 — 수동 보완 권장`
...
| 5 완료 보고 | ✅ | — |
| 6a Gap Analyze | ✅ | Design {avg}%, PRD {prd}%, Gap {gap_count}건 |
| 6b E2E Audit | ✅ | Route {route_cov}, Feature {feat_cov}, HIGH gap {high}건 |
| 7 Auto Correct | ✅ | Gap fix {iterated}건, E2E +{tests_added} tests |
...
3. `pending` 배치는 정상 실행
4. `phase6.status == "done"` → Phase 6 건너뜀 (Gap + E2E 모두)
5. `phase7.status == "done"` 또는 `"skipped"` → Phase 7 건너뜀 (Gap fix + E2E fix 모두)
...
| Phase | 예상 동작 |
|-------|----------|
| 6a Gap Analyze | gap-detector로 Design↔코드 + PRD Must Have 통합 분석 |
| 6b E2E Audit | 라우트 커버리지 + 기능 커버리지 + 품질 anti-pattern 감사 |
| 7a Gap Fix | pdca-iterator로 Gap 자동 수정 (최대 5회/Sprint) |
| 7b E2E Fix | HIGH gap 대상 E2E 테스트 자동 추가 |
...
8. **PDCA 통합**: Phase 6~7은 gap-detector/pdca-iterator/e2e-audit를 Master에서 직접 호출 (WT 재진입 불필요)
9. **E2E 통합**: Phase 6b E2E Audit + Phase 7b E2E 자동 보강으로 기능 커버리지 100% 목표
...
- Phase 6b E2E Audit은 dev 서버 없이도 실행 가능 (커버리지 분석은 코드 정적 분석)
- Phase 7b E2E 보강은 mock-only 테스트로 작성하여 CI safe (LLM/외부 API 의존 제거)
- 순차 Sprint (병렬 0)에서도 Phase 6~7은 동일하게 적용 — Sprint 단건 실행(/ax:sprint)에는 없는 통합 검증 가치
