...
2. **문서 위치 검증** — `docs/` 하위 PDCA 디렉토리에 규칙 외 파일이 있는지 확인:
   ...
   - **PDCA deviation 자동 보정 hint (C101 c)** — `features/` **외부**에서 sprint-prefixed 파일이 발견되면 위치 이탈로 분류해 안내. `features/` 내부의 `sprint-*.{type}.md`는 doc-governance.md §2 등록된 의도적 예외라 deviation 아님:
     ```bash
     # features/ 외부에 있는 sprint-* 파일만 deviation으로 카운트
     PDCA_DEVIATIONS=$(find docs/03-analysis docs/04-report -type f \
       \( -name 'sprint-*.analysis.md' -o -name 'sprint-*.report.md' \) \
       -not -path '*/features/*' 2>/dev/null | wc -l)
     if [ "$PDCA_DEVIATIONS" -gt 0 ]; then
       echo "⚠️  PDCA deviation ${PDCA_DEVIATIONS}건 발견 (features/ 외부의 sprint-*.{analysis,report}.md)"
       if [ -x scripts/sprint/rename-pdca-output.sh ]; then
         echo "   → 자동 보정: bash scripts/sprint/rename-pdca-output.sh"
         echo "   → 미리보기:  bash scripts/sprint/rename-pdca-output.sh --dry-run"
       else
         echo "   → 수동 git mv 또는 rename-pdca-output.sh 도입 필요 (C101)"
       fi
     fi
     ```
     이 hint는 _감지만_ 하고 자동 수정은 사용자 명시적 명령으로만 실행 (selfcheck 책임 범위 외).
     **예외 패턴 (deviation 아님)**: `docs/{type}/features/sprint-{N}.{type}.md`, `sprint-{N}-{N2}-pipeline.{type}.md`, `sprint-{N}-{slug}.{type}.md` — doc-governance.md §2 PDCA feature 폴더 변형 패턴 참조.
