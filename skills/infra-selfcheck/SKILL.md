...
2. **문서 위치 검증** — `docs/` 하위 PDCA 디렉토리에 규칙 외 파일이 있는지 확인:
   ...
   - **PDCA deviation 자동 보정 hint (C101 c)** — `sprint-{N}.{analysis,report}.md` 패턴 deviation이 발견되면 자동 보정 명령어 안내:
     ```bash
     PDCA_DEVIATIONS=$(find docs/03-analysis docs/04-report -type f \
       \( -name 'sprint-*.analysis.md' -o -name 'sprint-*.report.md' \) 2>/dev/null | wc -l)
     if [ "$PDCA_DEVIATIONS" -gt 0 ]; then
       echo "⚠️  PDCA deviation ${PDCA_DEVIATIONS}건 발견 (sprint-*.{analysis,report}.md)"
