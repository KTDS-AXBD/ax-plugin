...
CF_ACCOUNT="b6c06059b413892a92f150e5ca496236"  # ktds-axbd default
WORKER_SUBDOMAIN=""
OUTPUT_DIR=""
...
echo "  2. wrangler d1 create $PROJECT_NAME-db  # DATABASE_ID 발급"
echo "     → packages/api/wrangler.toml의 database_id 3곳 채우기"
echo "     → scripts/d1-migrate-remote.sh의 CLOUDFLARE_D1_DATABASE_ID 환경변수 설정"
echo "  3. gh secret set CLOUDFLARE_API_TOKEN   # GitHub Actions 배포 인증"
echo "  4. git push origin master               # CI/CD 자동 배포 (deploy.yml 트리거)"
echo "  5. bash scripts/smoke-test.sh           # 4-축 smoke 검증"
echo "  6. /ax:sprint 1 — 첫 Sprint 시동"
