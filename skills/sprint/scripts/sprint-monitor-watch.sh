...
#     signal도 IDLE_MIN(기본 10)분째 미진행 → rate limit 소진(5h:0%) 등
...
while true; do
  ...
  if [ "$stalled" -ge "$IDLE_SEC" ] && [ $((now - last_warn_ts)) -ge "$COOLDOWN_SEC" ]; then
    TAIL=$(tmux capture-pane -t "$SESSION" -p 2>/dev/null | tail -6)
    if ! echo "$TAIL" | grep -qE '✻|✢|✽|∗|·.*tokens|Running|interrupt|thinking'; then
      USAGE=$(echo "$TAIL" | grep -oE '5h:[0-9]+%' | tail -1)
      echo "⚠️ IDLE STALL: pane idle + signal ${IDLE_MIN}분+ 미진행 (STATUS=${STATUS}, CHECKPOINT=$(grep '^CHECKPOINT=' "$SIGNAL" | cut -d= -f2)${USAGE:+, usage ${USAGE}}) - rate limit 소진/턴 중단 의심. '작업 재개' nudge 검토"
      last_warn_ts=$now
    fi
  fi
