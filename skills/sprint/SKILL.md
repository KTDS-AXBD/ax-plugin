...
/ax:sprint start 102 F273
         ...
         ▼
┌─ Master 세션 ─────────────────────────────────────────────┐
│ Phase 0: Pre-flight 원격 고아 브랜치 사전 정리 (S253)     │
...
**Phase 0: Pre-flight — 원격 고아 브랜치 사전 정리** (S253 + 2026-04-19 Sprint 1 교훈):

sprint 생성 전에 **원격에 `sprint/N` 브랜치가 이미 존재하는지** 확인하고, 옛 merge된 잔재(고아 브랜치)면 선제 삭제한다. 원격에 다른 히스토리의 브랜치가 남아있으면 Sprint 완료 시점 push에서 **force-with-lease 충돌 → PR 미생성 → task-daemon FAILED** 체인으로 연쇄 실패한다.

> **실 사례** (2026-04-19 AIF-REQ-035 Sprint 1): 이전 `Recon-X MSA 재조정 Sprint 1` (PR #1 MERGED 2026-04-07)의 원격 `sprint/1` 브랜치가 `delete_branch_on_merge=false` 설정 탓에 잔존. 새 Sprint 1이 push 시점에 공통 조상 없는 원격 HEAD와 충돌하여 autopilot이 session-end 직전 중단. PR #9 수동 생성 + Signal 복구로 복귀.

```bash
# 1) 원격에 sprint/N 존재 여부 확인
REMOTE_SPRINT=$(git ls-remote origin "refs/heads/sprint/${N}" 2>/dev/null | awk '{print $1}')

if [ -n "$REMOTE_SPRINT" ]; then
  # 2) 공통 조상 확인 — 있으면 이어쓰기, 없으면 고아
  git fetch origin "sprint/${N}" 2>/dev/null
  if git merge-base --is-ancestor "origin/sprint/${N}" HEAD 2>/dev/null; then
    echo "✅ 원격 sprint/${N} 존재하나 HEAD의 선조 — 이어쓰기 가능 (삭제 불필요)"
  else
    # 3) 고아 브랜치 — PR 이력 조회로 merge 여부 확인 (squash-aware)
    PR_INFO=$(gh pr list --head "sprint/${N}" --state all --json number,state,mergedAt 2>/dev/null)
    PR_STATE=$(echo "$PR_INFO" | jq -r '.[0].state // empty')

    if [ "$PR_STATE" = "MERGED" ]; then
      echo "⚠️ 원격 sprint/${N} 고아 발견 — PR #$(echo "$PR_INFO" | jq -r '.[0].number') 이미 MERGED"
      echo "   → 사전 정리 필수 (push 충돌 방지)"
      # 사용자 확인 또는 자동 삭제 (AskUserQuestion 권장)
      git push origin --delete "sprint/${N}"
      echo "🧹 원격 sprint/${N} 삭제 완료"
    elif [ "$PR_STATE" = "OPEN" ]; then
      echo "🛑 원격 sprint/${N}에 OPEN PR 존재 — 사용자 확인 필요 (자동 삭제 금지)"
      # 중단하고 사용자에게 결정 위임
      return 1
    else
      echo "⚠️ 원격 sprint/${N} 고아 발견 — PR 이력 없음 또는 CLOSED"
      echo "   → 사용자 확인 후 진행 (AskUserQuestion)"
    fi
  fi
fi

# 4) (선택) 전체 고아 스캔 — git-orphan-scan.sh 설치 시 추가 감지
if [ -x "$HOME/scripts/git-orphan-scan.sh" ]; then
  ORPHAN_COUNT=$(bash "$HOME/scripts/git-orphan-scan.sh" --quiet 2>/dev/null | tail -1)
  [ -n "$ORPHAN_COUNT" ] && [ "$ORPHAN_COUNT" -gt 0 ] && \
    echo "ℹ️  전체 고아 리소스 ${ORPHAN_COUNT}건 감지 — git-orphan-clean.sh 별도 실행 권장"
fi
```

**판정 규칙**:
- 원격에 sprint/N 없음 → Phase 1로 진행
- 원격에 있으나 HEAD의 선조 → 이어쓰기 경로 (삭제 금지)
- 원격 고아 + PR MERGED → 자동 삭제 (Sprint 진행 전 정리)
- 원격 고아 + PR OPEN → **중단** (사용자 결정 필요, 자동 삭제 금지)
- 원격 고아 + PR 없음/CLOSED → AskUserQuestion
...
Monitor(
  ...
  persistent: true,
  command: "bash ${SKILL_DIR}/scripts/sprint-monitor-watch.sh ${PROJECT} ${N}"
)
```
...
- **Phase 0 Pre-flight 생략 금지** — 원격에 옛 `sprint/N` 고아 브랜치가 남아있으면 autopilot session-end 단계에서 force-with-lease push 차단 → PR 미생성 → task-daemon FAILED 연쇄. `delete_branch_on_merge=false` 레포에서 빈번 발생. S253 + 2026-04-19 AIF-REQ-035 Sprint 1 교훈
