...
**Step 1: F-item ↔ GitHub Issue 매칭 (2카테고리 분리)**
...
# 3. 1a 구조적 공백: SPEC 존재 but GitHub Issue 미생성
#    - 연속 범위 감지: F{start}~F{end} N건 형식
# 4. 1b 실시간 Drift: Issue 존재 but SPEC 상태 불일치
```

**1a. 구조적 공백 판정 기준** (SPEC 있음, Issue 없음):
- SPEC 존재 + GitHub 미등록 = ❌ 구조적 공백

**1b. 실시간 Drift 판정 기준** (Issue 있음, 상태 불일치):
- SPEC ✅ + GitHub CLOSED = OK
- SPEC 📋/🔧 + GitHub OPEN = OK
- SPEC ✅ + GitHub OPEN = ⚠️ Issue 미닫힘 (실시간 drift)
- SPEC 📋 + GitHub CLOSED = ⚠️ Issue 선닫힘 (실시간 drift)
